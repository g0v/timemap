...
var DataView = DomainObject.extend({
  ...
  // Return list of All DataView objects (toJSON'd)
  , List: function(callback){
    var backend = getBackend();
    backend.readdir('/', function(err, data) {
      var out = [];
      var data = data.filter(function(fileOrDirName) {
        return (fileOrDirName !== '.gitkeep');
      });
      function extract(ownerId, cb) {
        var vizs = DataView.getByOwner(ownerId, function(err, data){
          if(err === null || err === undefined) out.push(data);
          cb(err);
        });
      }
      async.eachSeries(data, extract, function(err, done) {
        callback(err, _.flatten(out));
      });      
    });
  }
});
